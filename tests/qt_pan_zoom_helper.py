...
### Re-writen in Qt just for fun

import os
from PySide2 import QtWidgets
import PySide2.QtCore
from PySide2.QtWidgets import QMainWindow, QWidget, QHBoxLayout,QVBoxLayout, QAction, QPushButton, QMenu, QTreeWidgetItem, QLineEdit, QTreeWidget, QLabel, QGroupBox, QInputDialog
...
class PAN_ZOOM_TOOL(QMainWindow):
    ...
    def load_ui(self):
        ...
        QLabel.text('PAN ZOOM TOOL')

        # Create a two row layout for camera name and button
        camera_setup_layout = QHBoxLayout(central_widget)

        self.set_camera_button = QPushButton('Set Cam')
        self.set_camera_button.clicked.connect(set_camera)

        self.camera_name_text_field = QLineEdit()
        self.camera_name_text_field.text('set_camera')
        self.camera_name_text_field.setEnabled(False)

        camera_setup_layout.addWidget(self.set_camera_button)
        camera_setup_layout.addWidget(self.camera_name_text_field)


        # Create Main Tools Layout
        layout_main_buttons = QHBoxLayout()
